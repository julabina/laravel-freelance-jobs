<template>
    <article :class="status === 'refused' ? 'relative flex justify-between p-6 border rounded border-opacity-60 border-gray-400 text-gray-400 pointer-events-none my-2' : 'relative flex justify-between p-6 border border-primary rounded border-opacity-60 my-2'">
        ...
        <div v-if="status !== 'refused'" class="flex flex-col items-end">
            <Link class="mb-2" v-if="mission.mission.status === 'open' || (mission.mission.status === 'granted' && mission.status === 'accepted')" :href="'/mission/' + mission.mission.id"><button class="btn-primary">
                voir
            </button></Link>
