<template>
    <article class="border-primary border rounded-lg border-opacity-25 p-6 mt-10">
        ...
        <div v-if="proposal.status === 'waiting'" class="flex flex-col md:flex-row justify-center mt-4">
            <button @click="negociated" class="btn-primary mx-1.5">Discuter avec le candidat</button>
            <button @click="refuse" class="btn-primary mx-1.5 mt-4 md:mt-0">Refuser le candidat</button>
        </div>
        <div v-if="proposal.status === 'negotiated'" class="flex flex-col md:flex-row justify-center mt-4">
